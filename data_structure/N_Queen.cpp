#include <bits/stdc++.h>
using namespace std;

//--------------------- define zone ------------------------
#define ll long long
#define db double
#define pi pair<int,int>
#define pis pair<string, string>
#define stp() ios_base::sync_with_stdio(false);cin.tie(0);
#define ending(x) if((x) == "x") break;
#define nloop(x) for(int (x) = 0;(x) < n;(x)++)
#define mloop(x) for(int (x) = 0;(x) < m;(x)++)
#define aloop(x) for(auto itr : (x))
#define all(x) (x).begin(), (x).end() 
#define ig() cin.ignore(256, '\n');
#define fs first
#define se second
#define DEBUG 1
const int big = 1e5+7;
const int INF = 1e9+7;


//--------------------- Variable zone ------------------------
int start,finish,cnt = 0;
queue<pi> q;
priority_queue<pi> pq;
vector<int> d8x = {1, -1, 0, 0, 1, -1, 1, -1};
vector<int> d8y = {0, 0, 1, -1, 1, -1, -1, 1};
vector<int> d4x = {0, 1, 0, -1};
vector<int> d4y = {1, 0, -1, 0};
vector<pi> node[big];
vector<int> dis(big,0);
vector<bool> vis(big,0);

//set up node (can be +level)
struct NODE {
   int i,j;
};

queue<NODE> qn;

//--------------------- Works zone ------------------------
bool chk(vector<vector<int> > &board, int row, int col, int n){
    for(int i = 0; i < row; i++) // chk same
        if(board[i][col] == 1) return false;
    for(int i = row, j = col; i >= 0 && j >= 0; --i, --j) // chk up left
        if(board[i][j] == 1) return false;
    for(int i = row, j = col; i >= 0 && j < n; --i, ++j) // chk up right
        if(board[i][j] == 1) return false;

    return true;
}   

void cal(vector<vector<int> > &board, int row, int n, int& cnt) {
    
    if(n == row) {
        cnt++;
        return ;
    }
    
    nloop(col) {
        if(chk(board, row, col, n)){
        board[row][col] = 1;
        cal(board, row + 1, n, cnt);
        board[row][col] = 0; // reset
        }
    }
}

void sol() {
    int n;
    cin >> n;
    vector<vector<int> > board(n, (vector<int>(n, 0)));
    int cnt = 0;
    cal(board, 0, n, cnt);
    cout << cnt << endl;
}

int main() {
    stp();
    int tt = 1;
    while(tt--) {
        sol();
    }  
    return 0;
}